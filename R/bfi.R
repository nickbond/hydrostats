bfi <- function(flow.ts, a = 0.975, ts = "mean") {
    ...
    if (ts == "daily") {
        ...
        if (ts == "mean") {
            ...
            return(list(prop.obs = prop.obs, MDF = mean(out[, 2], na.rm = T), Q50 = median(out[, 2], na.rm = T), mean.bf = mean(out[, 3], na.rm = T), mean.bfi = mean(out[, 
