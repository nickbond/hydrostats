...
ann.min.date<-as.Date(paste(names(ann.min.day.no),01,01,sep="-"))+ann.min.day.no

correct.ann.min.day<-day.dist(names(ann.min.day.no),ann.min.day.no)

#av.min.day<-mean(ann.min.day.no,na.rm=T)
#cv.min.day<-(sd(ann.min.day.no,na.rm=T)/mean(ann.min.day.no,na.rm=T))*100
