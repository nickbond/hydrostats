CTF <- function(flow.ts, threshold = 0.1) {
    ...
    if (length(flow.ts[, "Q"]) == 0 | sum(flow.ts[, "Q"]) == 0) {
        ...
        return(list(p.CTF = 1, avg.CTF = NA, med.CTF = NA, min.CTF = NA, max.CTF = NA))
    }
    ...
    return(list(p.CTF = p.CTF, avg.CTF = avg.CTF, med.CTF = med.CTF, min.CTF = min.CTF, max.CTF = max.CTF))
