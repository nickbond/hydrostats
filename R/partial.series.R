partial.series <-
function(flow.ts,ari=2,ind.days=7, duration=T, plot=F,volume=T) {
...
points(row.names(p.series),flow.ts[row.names(p.series),2],col="red")  
...
#spell.volumes<-aggregate(flow.ts[[2]],by=list(spell.factor),sum)
spell.volumes<-sapply(spells,sum)
spell.volumes<-spell.volumes[which(high.flow.runs$values==1)]

return(list(p.series=p.series,n.events=n.events, flow.threshold=flow.threshold, avg.duration=avg.duration, max.duration=max.duration, md.spell.volume=median(spell.volumes)-flow.threshold))
...
#return(list(n.events=n.events, flow.threshold=flow.threshold, avg.flow.duration=avg.duration, max.flow.duration=max.duration))
  }
